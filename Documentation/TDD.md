...
```mermaid
graph TD
    subgraph UI Layer [UI Layer (SwiftUI)]
        View[Fidget View]
    end

    subgraph Logic & Physics Layer (State Manager)
        Model[Fidget Model / State]
        DisplayLink[CADisplayLink / Timer]
    end

    subgraph Service Layer (Hardware Abstracted Managers)
...
4.  **Detent Calculation:**
    ...
    $$\text{Trigger Index } k = \lfloor \frac{\theta}{\Delta\theta_{detent}} \rfloor$$
