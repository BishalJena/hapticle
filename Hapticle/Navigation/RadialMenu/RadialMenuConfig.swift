...
/// Abstract interface for radial menu haptic clicks.
protocol HapticFeedback {
    func play(_ beat: HapticBeat)
}

/// Connects the radial menu to the production haptics engine.
struct HapticsAdapter: HapticFeedback {
    func play(_ beat: HapticBeat) {
        HapticsManager.shared.playClick(
            intensity: Double(beat.intensity),
            sharpness: Double(beat.sharpness)
        )
    }
}
